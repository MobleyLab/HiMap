...
def cluster_auto(data, ID_list, **kwargs):
    ...
    labels, mask, n_clusters_ = himap.dbscan(data)
...
def cluster_interactive(sim_data, ID_list):
    ...
    sub_arr, sub_ID = himap.sub_arrays(labels, sim_data, ID_list)
...
def clusters2optimize(sub_arr, sub_ID, **kwargs):
    '''
    Sends clusters for optimization. Takes himap.Optimize() kwargs.
    
        Parameters:
            sub_arr = dictionary of sub arrays of similarity for each cluster.
            output from himap.sub_arrays()
    ...
    if 'ref_ligs' not in kwargs:
        ...
        # Output jsons of cluster info
        himap.record_dicts(sub_ID, sub_refs = sub_refs)
