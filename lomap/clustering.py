...
def find_max_curvature(x, dists, **kwargs):
    ...
    if kwargs.get('savefigs') is True:
...
def plt_heatmap(data, ID_list, ax, fig, **kwargs):
    ...
    ax.set_xticklabels(ID_list[::tick_interval], rotation = 45)
...
def plt_cluster_regions(labels, ID_list, **kwargs):
    ...
    cmap = cm.get_cmap('CMRmap', len(unique_labels))
...
def cluster_interactive(sim_data, ID_list):
    ...
    x, dists = k_dist(data)
    auto_cutoff = find_max_curvature(x, dists, savefigs=False, verbose=True)
